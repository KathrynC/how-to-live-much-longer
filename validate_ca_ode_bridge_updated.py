...
# 1. Import ca_schema and patch BIN_SCHEMA before importing other modules
import ca_schema
# Load updated schema from file
with open("artifacts/ca_ode_validation/updated_schema_combined.json", "r") as f:
    updated = json.load(f)
# Replace BIN_SCHEMA in ca_schema module
ca_schema.BIN_SCHEMA = updated
print(f"Patched BIN_SCHEMA with updated schema ({len(updated)} variables)")

# Now import modules that depend on BIN_SCHEMA
...
from ca_schema import (
    ...
    CA_VAR_ORDER, bin_index, bin_count
)
...
def compare_exemplar_centers(bin_stats):
    ...
    for var_name, bins in bin_stats.items():
        comparison[var_name] = {}
        schema = ca_schema.BIN_SCHEMA[var_name]
